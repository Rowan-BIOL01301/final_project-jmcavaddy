...
#### Is there a relationship between the average age of an expedition group and whether the group successfully reached a peak?
...
members %>%
  ...
  mutate(success = if_else(success == TRUE, 1, 0)) %>% # change success column from logic to dbl for maths
  select(expedition_id, average_age, success) %>% # I only want to see these columns
  print()
...
expeditions %>%
  select(termination_reason, expedition_id) -> testing_tibble

expeditions %>%
  mutate(termination_reason_01 = if_else(str_detect(termination_reason, "Success"),
                                      1,
                                      0)) %>%
  select(termination_reason, expedition_id) -> probbbly_keeping_tibble

probbbly_keeping_tibble %>%
  anti_join(testing_tibble)
...
# expeditions %>%
#   mutate(termination_reason = if_else((termination_reason == str_detect("Success")),
#                                       1,
#                                       0)) %>%
